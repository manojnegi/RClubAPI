...
class SDHWindowsPlan extends APICallerAbstract {
 ...
 public function Add(array $params,array $country) {
     ...
     $endPoint = Http::prepare("singledomainhosting/windows/".$con."/add.json");
...
public function ReNew(array $params,$country){
   ...
   $endPoint = Http::prepare("singledomainhosting/windows/".$country."/renew.json");
...
public function Modify(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/modify.json');
...
public function EnableSSL(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/enable-ssl.json');
      $response = json_decode(Http::send($this->apicaller, $endPoint, $params, 'GET'));
        if (!is_object($response)) {
            throw new ResponseException(__METHOD__);
        }
...
public function DisableSSL(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/disable-ssl.json');
      $response = json_decode(Http::send($this->apicaller, $endPoint, $params, 'GET'));
        if (!is_object($response)) {
            throw new ResponseException(__METHOD__);
        }
...
public function Delete(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/delete.json');
      $response = json_decode(Http::send($this->apicaller, $endPoint, $params, 'GET'));
        if (!is_object($response)) {
            throw new ResponseException(__METHOD__);
        }
...
public function Details(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/details.json');
      $response = json_decode(Http::send($this->apicaller, $endPoint, $params, 'POST'));
        if (!is_object($response)) {
            throw new ResponseException(__METHOD__);
        }
...
public function GetOrderId(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/orderid.json');
      $response = Http::send($this->apicaller, $endPoint, $params, 'GET');
...
public function SearchOrder(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/search.json');
      $response = Http::send($this->apicaller, $endPoint, $params, 'GET');
...
public function GetUpgradePrice(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/modify-pricing.json');
      $response = Http::send($this->apicaller, $endPoint, $params, 'GET');
...
public function ChangeHostingPanelPassword(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/change-password.json');
      $response = Http::send($this->apicaller, $endPoint, $params, 'GET');
...
public function DNSRecord(array $params,$country){
      ...
      $endPoint = Http::prepare('singledomainhosting/windows/'.$country.'/dns-record.json');
      $response = Http::send($this->apicaller, $endPoint, $params, 'GET');
